...
import AccountNumberOcr from "./SimpleOCR";

export default function Home() {
  const [cart, setCart] = useState<Product[]>([]);
  const [mode, setMode] = useState<"main" | "purchase">("main");
        ...
        <div className="cart">
          ...
          <div className="checkout">
            <button onClick={() => {
              ...
              setMode('purchase');
            }} disabled={cart.length === 0 || (cart.reduce((total, item) => total + item.price, 0) > 500)}>{cart.reduce((total, item) => total + item.price, 0)>500 ? 'Over $500!' : 'Checkout'}</button>
          </div>
        </div>
      </>
      }
      {mode === 'purchase' && <><AccountNumberOcr setMode={setMode} cart={cart} /></>}
    </>
  );
}
